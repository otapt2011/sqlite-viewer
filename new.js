...
SELECT id, 0, 0, 'scheduled' FROM matches;`;
    ...
    async function createFIFA2026Database() {
        if (!window.JaferSQL) {
            throw new Error('JaferSQL is not loaded. Make sure jafersql.js is included before new.js');
        }
        const db = await window.JaferSQL.jaferInit();
        // Run schema (creates tables, indexes, views)
        db.jaferRun(SCHEMA_SQL);
        // Insert data
        db.jaferRun(TEAMS_INSERT);
        db.jaferRun(GROUPS_INSERT);
        db.jaferRun(CITIES_INSERT);
        db.jaferRun(STADIUMS_INSERT);
        db.jaferRun(STAGES_INSERT);
        db.jaferRun(MATCHES_INSERT);
        db.jaferRun(MATCH_SCORES_INSERT);
        return db;
    }
