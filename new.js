/**
 * new.js – FIFA 2026 SQLite Database Creator (Corrected)
...
(function() {
    ...
    // ============================================================
    // 1. COMPLETE SQL SCHEMA (all tables, indexes, views)
    // ============================================================
    const SCHEMA_SQL = `
        -- Core Tables
        ...
        CREATE INDEX IF NOT EXISTS idx_bracket_rules_stage ON bracket_rules(stageId);
        CREATE INDEX IF NOT EXISTS idx_bracket_rules_date ON bracket_rules(date);

        -- Views (all original + new)
        ...
        CREATE VIEW IF NOT EXISTS group_standings_full AS
        WITH group_teams AS (
        ...
        CREATE VIEW IF NOT EXISTS match_details_sorted AS
        SELECT 
            md.*,
            ...
            substr(md.date, -4) || 
            CASE 
                WHEN md.date LIKE '%January%'   THEN '01'
                WHEN md.date LIKE '%February%'  THEN '02'
                WHEN md.date LIKE '%March%'     THEN '03'
                WHEN md.date LIKE '%April%'     THEN '04'
                WHEN md.date LIKE '%May%'       THEN '05'
                WHEN md.date LIKE '%June%'      THEN '06'
                WHEN md.date LIKE '%July%'      THEN '07'
                WHEN md.date LIKE '%August%'    THEN '08'
                WHEN md.date LIKE '%September%' THEN '09'
                WHEN md.date LIKE '%October%'   THEN '10'
                WHEN md.date LIKE '%November%'  THEN '11'
                WHEN md.date LIKE '%December%'  THEN '12'
            END || '-' ||
            substr('0' || trim(substr(md.date, instr(md.date, ' ') + 1, 2)), -2) AS sort_date
        FROM match_details md
        LEFT JOIN teams home ON home.name = md.homeTeam
        LEFT JOIN teams away ON away.name = md.awayTeam;

        -- Corrected knockout_matches_resolved view (fixed split_letters)
        CREATE VIEW IF NOT EXISTS knockout_matches_resolved AS
        WITH
            match_results AS (
                SELECT 
                    m.id AS matchId,
            ...
            all_rules AS (
                SELECT matchNumber, 'home' AS side, homeRule AS rule FROM bracket_rules
                UNION ALL
                SELECT matchNumber, 'away' AS side, awayRule AS rule FROM bracket_rules
            ),
            split_letters(rule_str, pos, letter) AS (
                SELECT rule, 1, substr(rule, 1, 1) FROM all_rules WHERE rule GLOB '3[A-Z]*'
                UNION ALL
                SELECT rule_str, pos+1, substr(rule_str, pos+1, 1)
                FROM split_letters
                WHERE pos < length(rule_str)
            ),
            resolved AS (
                SELECT 
                    ar.matchNumber,
                    ar.side,
                    CASE
                        WHEN ar.rule GLOB '1[A-Z]' THEN
                            (SELECT teamId FROM group_standings_full WHERE groupName = substr(ar.rule,2) AND rank = 1 LIMIT 1)
                        WHEN ar.rule GLOB '2[A-Z]' THEN
                            (SELECT teamId FROM group_standings_full WHERE groupName = substr(ar.rule,2) AND rank = 2 LIMIT 1)
                        WHEN ar.rule GLOB 'W[0-9]*' THEN
                            (SELECT winnerId FROM match_results WHERE matchId = CAST(substr(ar.rule,2) AS INTEGER))
                        WHEN ar.rule GLOB 'RU[0-9]*' THEN
                            (SELECT loserId FROM match_results WHERE matchId = CAST(substr(ar.rule,3) AS INTEGER))
                        WHEN ar.rule GLOB '3[A-Z]*' THEN (
                            SELECT teamId FROM group_standings_full gs
                            WHERE gs.rank = 3
                                AND gs.groupName IN (SELECT letter FROM split_letters WHERE rule_str = ar.rule)
        ...
        CREATE VIEW IF NOT EXISTS unified_matches AS
        SELECT 
            id,
            date,
            time,
        ...
        SELECT
            ...
            date,
            time,
            COALESCE(homeTeamName, homeRule) AS homeTeam,
            COALESCE(awayTeamName, awayRule) AS awayTeam,
            ...
            homeRule,
            awayRule
        FROM knockout_matches_resolved
        WHERE stageId >= 2;
    `;
...
(104,'Sunday 19 July 2026','22:00',NULL,NULL,NULL,7,6,6);`;
    ...
    const BRACKET_RULES_INSERT = `INSERT INTO bracket_rules (matchNumber, stageId, date, time, stadiumId, cityId, homeRule, awayRule) VALUES
(73,2,'2026-06-28','22:00',4,4,'2A','2B'),
(74,2,'2026-06-29','23:30',9,9,'1E','3ABCDF'),
(75,2,'2026-06-30','04:00',7,7,'1F','2C'),
(76,2,'2026-06-29','20:00',12,12,'1C','2F'),
(77,2,'2026-07-01','00:00',10,10,'1I','3CDFGH'),
(78,2,'2026-06-30','20:00',6,6,'2E','2I'),
(79,2,'2026-07-01','04:00',1,1,'1A','3CEFHI'),
(80,2,'2026-07-01','19:00',13,13,'1L','3EHIJK'),
(81,2,'2026-07-02','03:00',14,14,'1D','3BEFIJ'),
(82,2,'2026-07-01','23:00',5,5,'1G','3AEHIJ'),
(83,2,'2026-07-03','02:00',4,4,'2K','2L'),
(84,2,'2026-07-02','22:00',3,3,'1H','2J'),
(85,2,'2026-07-03','06:00',8,8,'1B','3EFGIJ'),
(86,2,'2026-07-04','01:00',10,10,'1J','2H'),
(87,2,'2026-07-04','04:30',15,15,'1K','3DEIJL'),
(88,2,'2026-07-03','21:00',16,16,'2D','2G'),
(89,3,'2026-07-05','00:00',9,9,'W74','W77'),
(90,3,'2026-07-04','20:00',11,11,'W73','W75'),
(91,3,'2026-07-05','23:00',6,6,'W76','W78'),
(92,3,'2026-07-06','03:00',1,1,'W79','W80'),
(93,3,'2026-07-06','22:00',10,10,'W83','W84'),
(94,3,'2026-07-07','03:00',14,14,'W81','W82'),
(95,3,'2026-07-07','19:00',13,13,'W86','W88'),
(96,3,'2026-07-07','23:00',8,8,'W85','W87'),
(97,4,'2026-07-09','23:00',7,7,'W89','W90'),
(98,4,'2026-07-10','22:00',4,4,'W93','W94'),
(99,4,'2026-07-12','00:00',15,15,'W91','W92'),
(100,4,'2026-07-12','04:00',16,16,'W95','W96'),
(101,5,'2026-07-14','22:00',10,10,'W97','W98'),
(102,5,'2026-07-15','22:00',13,13,'W99','W100'),
(103,6,'2026-07-19','00:00',15,15,'RU101','RU102'),
(104,7,'2026-07-19','22:00',6,6,'W101','W102');`;
...
SELECT id, 0, 0, 'scheduled' FROM matches;`;
    ...
    async function createFIFA2026Database() {
        ...
        try {
            ...
            console.log('FIFA 2026 database created successfully with all tables, views, and bracket rules.');
            return db;
        } catch (err) {
