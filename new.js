    ...
    const SCHEMA_SQL = `
        ...
        WITH
            ...
            split_letters(rule_str, pos, letter) AS (
                SELECT rule, 1, substr(rule, 1, 1) FROM bracket_rules WHERE rule GLOB '3[A-Z]*'
                UNION ALL
                ...
                SELECT 
                    ...
                    CASE
                        ...
                        WHEN br.homeRule GLOB '3[A-Z]*' THEN (
                            ...
                            WHERE gs.rank = 3
                                AND gs.groupName IN (SELECT letter FROM split_letters WHERE rule_str = br.homeRule)
                ...
                SELECT 
                    ...
                    CASE
                        ...
                        WHEN br.awayRule GLOB '3[A-Z]*' THEN (
                            SELECT teamId FROM group_standings_full gs
                            WHERE gs.rank = 3
                                AND gs.groupName IN (SELECT letter FROM split_letters WHERE rule_str = br.awayRule)
                            ORDER BY gs.points DESC, gs.goalDifference DESC, gs.goalsFor DESC
                            LIMIT 1
                        )
                        ELSE NULL
